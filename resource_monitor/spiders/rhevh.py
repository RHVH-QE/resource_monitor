...
class RhevhSpider(CrawlSpider):
    name = 'rhevh7'
    allowed_domains = [
        'brewweb.engineering.redhat.com', 'download.eng.bos.redhat.com',
        'download.devel.redhat.com', 'download.engineering.redhat.com'
    ]

    start_urls = [
        'https://brewweb.engineering.redhat.com/brew/packageinfo?packageID=36140',
    ]

    rules = (Rule(
        LxmlLinkExtractor(
            restrict_xpaths=('//a[contains(@href, "buildinfo")]', )),
    ...
    def parse_single_build_page(self, response):
        ...
        if item['build_status'] == 'complete':
            item['build_tag'] = response.xpath(
                '//a[contains(@href, "taginfo?tagID")]/text()').extract()[0]
            item['build_iso'] = response.xpath(
                '//a[text()="download"]/@href').re('(.+\.iso|.+\.noarch.rpm)')[
                    0]
            build_log_url = response.xpath(
                '//a[text()="build logs"]/@href').extract()[0]
            ...
            item[
                'build_iso'] = 'No ISO available, due to build status is not complete'
            return
...
class Rhevh6Spider(RhevhSpider):
    name = 'rhevh6'
    start_urls = [
        'https://brewweb.engineering.redhat.com/brew/packageinfo?packageID=33636',
    ]
...
class RhevhNGN36Spider(RhevhSpider):
    name = "rhevh36ngn"
    start_urls = [
        'https://brewweb.engineering.redhat.com/brew/packageinfo?packageID=58439',
    ]
    ...
    def parse_single_build_page(self, response):
        ...
        if item['build_status'] == 'complete':
            item['build_tag'] = response.xpath(
                '//a[contains(@href, "taginfo?tagID")]/text()').extract()[0]
            item['build_update_rpm'] = response.xpath(
                '//a[text()="download"]/@href').re(
                    '(.+redhat-virtualization-host-image-update.+)')[0]
            item['build_ks'] = response.xpath(
                '//a[text()="download"]/@href').re('(.+\.ks)')
            item['build_squashfs_img'] = response.xpath(
                '//a[text()="download"]/@href').re('(.+\.squashfs)')[0]
            item['build_downloaded'] = False
            ...
            item['build_tag'] = 'No Tag Found'
            item[
                'build_squashfs_img'] = 'No ISO available, due to build status is not complete'
            return
